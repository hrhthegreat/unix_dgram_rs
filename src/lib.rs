                ...
                while !stop_for_thread.load(Ordering::Relaxed) {
                    match recv_sock.recv_from(&mut buf) {
                        Ok((size, addr)) => {
                            ...
                            let path = addr.as_pathname().map(|p| p.to_string_lossy().to_string());
                            let packet = RecvPacket {
                                data: Buffer::from(buf[..size].to_vec()),
                                path,
                            };
                            let status =
                                tsfn.call(Ok(packet), ThreadsafeFunctionCallMode::NonBlocking);
                            if status != Status::Ok {
                                return;
                            }
                        }
...
mod imp {
    ...
    fn not_supported() -> Error {
        Error::from_reason(
            "ENOTSUP: unix_dgram is not supported on win32 in this build".to_string(),
        )
    }
